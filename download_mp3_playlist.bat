...
youtube-dl.exe --geo-bypass --yes-playlist --abort-on-unavailable-fragment --newline --add-metadata --embed-thumbnail -x --audio-format mp3 -w -c -a "download_list_playlists.txt" --output "downloads\%%(playlist_title)s\%%(title)s.%%(ext)s" --download-archive "archive.txt"
