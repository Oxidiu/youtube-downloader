...
youtube-dl.exe --geo-bypass --yes-playlist --abort-on-unavailable-fragment --newline --write-thumbnail --add-metadata --embed-thumbnail -x --audio-format mp3 -w -c -a "download_list.txt" --output "downloads\%%(title)s.%%(ext)s" --download-archive "archive.txt"
